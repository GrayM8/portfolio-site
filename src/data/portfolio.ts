...
export const PORTFOLIO: Portfolio = {
  ...
  featured: [
    ...
    },
    {
      ...
      detailBullets: [
        ...
        "Per-field BACKEND TODOs in the type layer let the frontend render unwired fields as `null` today and light up automatically once `dashd` publishes them — energy-laps, lap times, brake bias, and TC/regen flags all wired against the eventual schema without rewrites.",
        "Demo mode (`D` key toggle) — a parallel `useDemoData` hook synthesizes the full `DashMessage` shape so the dash can be presented off-car, tested without the broker, or shown to engineers and judges at identical visual fidelity to live mode.",
  ...
  projects: [
    ...
    },
    {
      title: "Invest Check",
      slug: "invest-check",
      tagline: "Marketplace for verified, reconciled trader histories — built as a freelance contractor.",
      tech: [
        "Next.js 16",
        "TypeScript",
        "Drizzle",
